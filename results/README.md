...
`table_Qi.csv` provides instance-wise computational results for all 144 large-scale `Qi/` instances referenced in Section 7.2 of the paper. A LaTeX version of this table can be regenerated by running `bash scripts/generate_tables.sh`, which writes the output to `scripts/table/table_Qi.tex`. 
