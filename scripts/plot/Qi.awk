BEGIN{
    ...
    pos += 1; col = "SepaRound";      PrintName[col][str1] = "\\texttt{SepaRound}";     PrintName[col][str2] = "SepaRound"
    pos += 1; col = "pmpT";           PrintName[col][str1] = "\\texttt{pmpT}";          PrintName[col][str2] = "pmpT"
    pos += 1; col = "lp_iter";        PrintName[col][str1] = "\\texttt{ItCnt}";         PrintName[col][str2] = "ItCnt"
    pos += 1; col = "CommonIndex";    PrintName[col][str1] = "\\texttt{CommonIndex}";   PrintName[col][str2] = "CommonIndex"
...
/Solve Time:/ {
    if (data[id][setting]["tstatus"] == "TIME_LIMIT")
        data[id][setting]["time"] = TIMELIMIT
    else
...
/Number of Nodes:/  {
    data[id][setting]["node"] = $4
}
/Root Best Bound/   { data[id][setting]["rBB"] = $4 }
/Root Best Integer:/{ data[id][setting]["rBInt"] = $4 }
/Best Bound:/       { data[id][setting]["BB"] = $3 }

/^Separation Round:/ { data[id][setting]["SepaRound"] = $4+$6+$8+$10+$12 }
/PMP/ { data[id][setting]["pmpT"] = $5+$9+$13+$17+$21 }

/^Best Integer:/{
    data[id][setting]["BInt"] = $3
    data[id][setting]["gap"] = (data[id][setting]["BB"] - data[id][setting]["BInt"]) / data[id][setting]["BB"] * 100
    if (data[id][setting]["obj"] > 0)
        data[id][setting]["rgap"] = (data[id][setting]["rBB"] - data[id][setting]["obj"]) / data[id][setting]["rBB"] * 100
}
...
/^Best Integer:/{
    if (is_two_stage == "true") {
        if (data[id][setting]["PhaseOneBB"] - data[id][setting]["obj"] < 0.01)
            data[id][setting]["rgap"] = 100
        else
            data[id][setting]["rgap"] = (data[id][setting]["PhaseOneBB"] - data[id][setting]["rBB"]) / (data[id][setting]["PhaseOneBB"] - data[id][setting]["obj"]) * 100
    } else {
        if (data[id][setting]["obj"] > 0)
            data[id][setting]["rgap"] = (data[id][setting]["rBB"] - data[id][setting]["obj"]) / data[id][setting]["rBB"] * 100
    }
...
END {
    ...
    if (format == "tex") {
        printf("\\resizebox{\\textwidth}{0.5\\textheight}{\n")
    ...
    if (format == "tex") { printf("%-20s", ""); printf("& & &") }
    for (ind_setting = 1; ind_setting <= length(Arr_setting); ind_setting++) {
        setting = Arr_setting[ind_setting]
        if (setting == 0) continue
        for (ind_col = 1; ind_col <= length(Col); ind_col++) {
            col = Col[ind_col]
            if (format == "tex") printf("%s%8s", col_sep, PrintName[col][format])
        }
    }
    if (format == "tex") { printf("%s\n", row_sep); printf("\\midrule\n") }
    ...
    for (ind_id = 1; ind_id <= length(Arr_id); ind_id++) {
        ...
        for (ind_setting = 1; ind_setting <= length(Arr_setting); ind_setting++) {
            ...
            if (setting ~ /MO1/) { rBB_MO1 = data[id][setting]["rBB"]; obj_MO1 = data[id][setting]["obj"] }
        }
        printf("%s\n", row_sep)
        ...
        for (ind_setting = 1; ind_setting <= length(Arr_setting); ind_setting++) {
            ...
            for (ind_col = 1; ind_col <= length(Col); ind_col++) {
                ...
                if (col ~ /OR/) {
                    ...
                    printf("%s", col_sep)
                    ind_or = 1
                    tmpcol = arr_or[ind_or]; val = CalculateAverage(data, Arr_id_ave, setting, tmpcol, 1)
                    format_print(tmpcol, val, "", 1)
                } else if (col ~ /obj/ || col ~ /status/) {
                    printf("%s%8s", col_sep, "")
                    continue
                } else if (col ~ /node/) {
                    val = CalculateAverage(data, Arr_id_ave, setting, col, 1)
